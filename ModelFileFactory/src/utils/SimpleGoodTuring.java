...
/**
 * I copied this class from Stanford parser. the reason I copied it since I don't change anything about stanford parser but
 * I have to change some variabls and functions from private to public.
...
public class SimpleGoodTuring {
  ...
  private void compute() {
    ...
    smoothedProb = new double[rows];
        
    for (j = 0; j < rows; ++j) bigN += r[j] * n[j]; // count all items
    next_n = row(1); //next_n is the next one which has the countOfcount as row(countOfcount), starting as 1. 
    pZero = (next_n < 0) ? 0 : n[next_n] / (double) bigN;
    for (j = 0; j < rows; ++j) {
      i = (j == 0 ? 0 : r[j - 1]);
      if (j == rows - 1) //the last row
        k = (double) (2 * r[j] - i);
      else
        k = (double) r[j + 1];
      z[j] = 2 * n[j] / (k - i);
    ...
    for (j = 0; j < rows; ++j) {
      ...
      //row(r[j] + 1) < 0, it means that it passes the last row. since the loop will never reach r[i]==r[j]+1
      if (row(r[j] + 1) < 0)
        indiffValsSeen = true;
      if (!indiffValsSeen) {
    ...
    for (j = 0; j < rows; ++j)
      smoothedProb[j] = (1 - pZero) * rStar[j] / bigNPrime;
  }
  ...
  void print() {
    ...
    for (i = 0; i < rows; ++i)
      System.out.printf("%6d %6d %8.4g %8.4g%n", r[i], n[i], 1.0 * r[i] / bigN, smoothedProb[i]);
  }
   ...
   * Helper to readInput().
   */
  static int[] integerList2IntArray(List<Integer> integers) {
   ...
   * and one type occurs 8 times, for a total count of 52.  This input will
