	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		if(inputFile.isDirectory()){
			...
			for(int i=0;i<listOfFiles.length;i++){
				...
				PrintWriter pwPosSynDep = null; 
				PrintWriter pwIndModel = null;
				try {
					pwPosSynDep = new PrintWriter(new FileWriter(new File(outputDir,input.getName()+".possyndep"),true));
					pwIndModel = new PrintWriter(new FileWriter(new File(outputDir,input.getName()+".indmodel"),true));
					...
					processReutersCorpus.convertIndStruct(pwIndModel);
					processReutersCorpus.buildPosSynDep(pwPosSynDep);
				} catch (IOException e) {
			...
			PrintWriter pwPosSynDep = null; 
			try {
				pwPosSynDep = new PrintWriter(new FileWriter(new File(outputDir,input.getName()+".possyndep")));
				processReutersCorpus.printIndStructList();
				processReutersCorpus.fillGenderHM();
				processReutersCorpus.fillNumberHM();
				processReutersCorpus.buildGenderDep();
