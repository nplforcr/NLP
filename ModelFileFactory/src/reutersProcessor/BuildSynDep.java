	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		if(inputFile.isDirectory()){
			...
			for(int i=0;i<listOfFiles.length;i++){
				...
				PrintWriter pwSynDep = null; 
				try {
					pwSynDep = new PrintWriter(new FileWriter(new File(outputDir,input.getName()+".syndep"),true));
					...
					processReutersCorpus.buildSynDep(pwSynDep);
				} catch (IOException e) {
			...
			PrintWriter pwSynDep = null; 
			try {
				pwSynDep = new PrintWriter(new FileWriter(new File(outputDir,input.getName()+".syndep")));
				processReutersCorpus.printIndStructList();
				processReutersCorpus.fillGenderHM();
				processReutersCorpus.fillNumberHM();
				processReutersCorpus.buildGenderDep();
