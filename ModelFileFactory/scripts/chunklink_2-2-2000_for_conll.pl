...
"call as:                                                                                      \n";
	print
"  chunklink.pl <options> /cdrom/treebank/combined/wsj/0?/wsj_0???.mrg | more                  \n";
	print
"                                                                                              \n";
	print
"options:                                                                                      \n";
...
sub new {
	...
	if ( defined( $obj->{reference} ) ) {
		if ( defined( $obj->{reference}->{trace} )
			&& ref( $obj->{reference}->{trace} ) eq 'ARRAY' )
		{
			$obj->{reference}->{trace} =
			  [ $obj, @{ $obj->{reference}->{trace} } ]
			  ;                    # filler points to trace
		}
		else {
...
foreach $file (@ARGV) {    # loop: for each treebank file
	...
	while ( defined( $sentence = <INPUT> ) ) {  # loop: until end of file
		...
		if (
			defined( $result->{function} )      # delete outermost parentheses
...
#######
#####
# A mystery for me is that I don't understand why the begining of the input must have two brackets!!!!!!!!!. 
# I will look at this later. Now, what I can do is that add brackets to each stanford parser input. 
###
sub start_read {

	# skip all lines not of the form "   (..." (e.g. blank lines)
	while ( $sentence !~ /^\s*\((.*)$/ && defined( $sentence = <INPUT> ) ) { }
	chop($sentence);
...
sub read_sentence {
	...
	my $mydepth =
	  shift(@_);    # at which bracket depth the constituent starts/ends
	my @store =
	  ();    # references to the daughters of the constituent will go in here

	while ( $depth > $mydepth )
	{        # while closing bracket of constituent is not yet found
		...
		if ( $sentence =~ /^\(([^ ()]+) ([^ ()]+)\)(.*)$/ )
		{    # (NN man)           # terminal node
			$tag      = $1;
			$word     = $2;
			$sentence = $3;
			if ( $tag eq '-NONE-' ) {    # null element
				if ( $word =~/^(\*T\*|\*U\*|\*NOT\*|\*RNR\*|\*ICH\*|\*EXP\*|\*PPA\*|\*\?\*|\*)(.*)$/)
				{                        # trace
					my $kind = $1;
					my $rest = $2;
					if ( defined($rest) && $rest =~ /^-([0-9]+)$/ )
					{                    # co-referenced
						if ( defined( $corefs{$1} ) )
						{    # co-reference already defined (backward reference)
							push(
								@store,
								'trace'->new(
									'NOFUNC', $kind, undef, $corefs{$1}
								)
							);
						}
						else
						{    # co-reference not yet defined (forward reference)
							my $ref =
							  'trace'->new( 'NOFUNC', $kind, undef, undef );
							push( @store, $ref );
							$tracerefs{$1} =
							  $ref;    # store in hash for later processing
						}
					}
				...
				$word =~ s/-RCB-/}/g;
				push( @store,
					'terminal'->new( $word_num++, 'O', $tag, $word, 'NOFUNC' )
				);
				$word_number++;
			}
		}
		elsif ( $sentence =~ /^\((\S+)(.*)$/ ) #\S  Match non-whitespace character
		{    # (XP                             # beginning of non-terminal node
			my $index;
			my $sublabel = $1;
			$sentence = $2;
			$depth++;
			...
			if ( defined($index) ) { # was coreferenced
				if ( defined( $tracerefs{$index} ) )
				{    # trace had been found before filler (forward reference)
					$tracerefs{$index}->{reference} =
					  $ref;    # trace points to filler
					$ref->{trace} =
					  [ $tracerefs{$index} ];    # filler points to trace
				}
		...
		#	}
			...
			print die
			  "$filenumber $sentence_number ERROR: No match(2) $sentence!\n";
		}
	}
	if ( $depth == $mydepth )
	{    # if corresponding closing bracket has been found
		return non_terminal->new( $label, \@store )
		  ;    # return reference to constituent
	}
	else {
...
sub verbs_or_adverbs_in_front {
	if (
		@lastnonref + @advps > 0    # at least one verb or adverb in front
		&& $i == @lastnonref + @advps
	  )
	{                               # only verbs and adverbs in front
		return 1;
	}
	else {
		return 0;
...
sub simple_prune {
	splice( @$daughters, $i, 1, @{ $daughters->[$i]->{daughters} } )
	  ;                             # insert contents of ADJP etc.
	$i--;
}

sub prune_drop_first {
	splice( @$daughters, $i, 1, @{$s_daughters}[ 1 .. $#{$s_daughters} ] )
	  ;                             # insert all but first daughter of S
	$i--;
}
...
sub prune_s_in_vp_condition {
	if (
		...
		&& $daughters->[$i]->{daughters}->[0]->{function} =~
		/-SBJ/    # 1) subject: include S-NOM-SBJ
	  )
	{
		return 1;
	}
...
sub prune_s_in_vp {
	...
	if ( prune_s_in_vp_predicative_condition() ) {
		$s_daughters->[0]->{function} =~
		  s/-SBJ//;    # change function to "direct object": also for S-NOM-SBJ
		simple_prune();
	}
	elsif ( prune_s_in_vp_empty_subject_condition() ) {
		if ( defined( $s_daughters->[0]->{daughters}->[0]->{reference} ) )
...
sub prune_s_in_vp_empty_subject_condition {
	if (
		$prune_s_in_vp_empty_subject_flag
		&& @{ $s_daughters->[0]->{daughters} } ==
		1    #    subject has only one daughter
		&& ref( $s_daughters->[0]->{daughters}->[0] ) eq
		'trace'    #    which is trace
...
sub np_condition {
	if (
		$xp eq 'NP'                                 # special case of NPs
		&& $i != @$daughters - 1
		&& defined( $daughters->[ $i + 1 ]->{function} )
		&& $daughters->[ $i + 1 ]->{function} =~
		/^NP/                                       # no directly following NP
		&& defined( $daughters->[$i]->{daughters} )
		&& defined( $daughters->[$i]->{daughters}->[-1] )
		&& defined( $daughters->[$i]->{daughters}->[-1]->{pos_tag} )
		&& $daughters->[$i]->{daughters}->[-1]->{pos_tag} eq 'POS'
	  )
	{                                               # no possessive NP
		return 1;
	}
...
sub check_non_terminal {
	if ( exists( $prune_always{$sub_labelxp} ) ) {
		...
		exists( $prune_if_infrontof_head{$xp}{$sub_labelxp} )
...
#	   && @lastnonref==0  # no possible head-word found to date: would result in "a [UCP state and local] [NP utility]"
	...
	{ # no possible head-constituent found to date: don't prune "P.V., 61 years old, ..."
		simple_prune();
	}
	elsif (
		   $prune_vp_in_vp_flag
		&& $xp eq 'VP'
		&& $sub_labelxp eq 'VP'    # VP in VP
		&& verbs_or_adverbs_in_front()
	  )
	{    #    verbs or adverbs in front
		simple_prune();
	}
...
sub prune_advp_in_vp {
	if (   $prune_advp_in_vp_flag
		...
		&& @advps > 0
		&& @lastnonref > 0 )
	{                           # there is an ADVP in a VP
		my $i   = 0;
		my $add = 0;
		my $index;
		while ( $i < @advps && $advps[$i] < $lastnonref[-1] )
		{                       # ADVP is in front of last verb
			$index = $advps[$i] + $add;
			if ( ref( $daughters->[$index] ) eq 'non_terminal' )
			{                   # prune only ADVPs not pure RBs
				$add +=
				  @{ $daughters->[$index]->{daughters} } -
				  1;            # -1: original reference is replaced
				splice( @$daughters, $index, 1,
					@{ $daughters->[$index]->{daughters} } )
				  ;             # insert contents of ADVP
			}
			$i++;
		}
	}
...
sub flatten {
	...
	if ( ref($res) eq 'terminal' ) {    # stop at word level
		return $res;
	}
	...
	{                                   # XP
		...
		for ( $i = 0 ; $i < @$daughters ; $i++ ) {    #loop on daughters

			# For terminals, initialize the IOB tag in a special field
			if ( ref( $daughters->[$i] ) eq 'terminal' ) {    # terminal
				$daughters->[$i]->{iob_tag_inner} = "I-$xp-$chunk_number";
			}
			...
			if ( $daughters->[$i]->{head_comp} eq 'h' ) {
				...
				{    # NOFUNC
					$daughters->[$i]->{function} =
					  $res->{function};    # copy function to head(s)
				}
				else {
					$daughters->[$i]->{function} =
...
sub cut_traces {
	...
	{    # stop at word or trace level
	...
	elsif ( ref($res) eq 'non_terminal' ) {    # XP
		my $daughters = $res->{daughters};
		for ( $i = 0 ; $i < @$daughters ; $i++ ) {
		...
		for ( $i = 0 ; $i < @$daughters ; $i++ ) {
			if (
				ref( $daughters->[$i] ) eq 'trace'    # trace
				|| (
					ref( $daughters->[$i] ) eq
					'non_terminal'                    # or empty constituent
		...
		if ( $ref->{iob_tag_inner} =~ /^I-([A-Z]+)-([0-9]+)$/ ) {    # inside
		  ...
		  # lex_head is head of following NP (same as lex_head of previous word)
			...
			if ( $old_nr != $nr ) {                 # chunk boundary
				...
				if ( defined($prev_ref) ) {
					$prev_ref->{iob_tag_inner} =~
					  s!^I-!B-!;                       # beginning of chunk
			...
			die
"File $filenumber: Error: no match $ref->{iob_tag_inner}, $ref->{word}!\n";
		}
	}
...
sub print_flatten {
	...
	for ( $i = 0 ; $i < @flattened ; $i++ ) {    # loop on words
		if ($sent_each_word) {
			...
			if ( $opt_B eq 'Begin' ) {
				...
				if (
					...
					&& (
						...
						|| substr( $flattened[ $i - 1 ]->{iob_tag_inner}, 2 ) ne
						$1
					)
				  )
				{                  # different category
		...
		if ( defined( $flattened[$i]->{trace} ) ) {
			...
			for ( $l = 0 ; $l < @$trace_array ; $l++ ) {
			  ...
			  # break circular references to allow for proper garbage collection
				undef
				  $trace->{reference};   # delete reference from trace to filler
			}
			undef $flattened[$i]
			  ->{trace};    # delete references from filler to traces
		}
...
sub print_list {
	...
	foreach $r (@r) {
		print
"terminal->{number}=$r->{number}, {iob_tag}=$r->{iob_tag}, {pos_tag}=$r->{pos_tag}, {word}=$r->{word}, {function}=$r->{function}, {head_comp}=$r->{head_comp}, {lex_head}=";
...
"terminal->{number}=$r->{number}, {iob_tag}=$r->{iob_tag}, {pos_tag}=$r->{pos_tag}, {word}=$r->{word}, {function}=$r->{function}, {head_comp}=$r->{head_comp}, {lex_head}=";
		...
		print
		  "trace->{function}=$r->{function}, {kind}=$r->{kind}, {lex_head}=";
		print_lex_head( $r->{lex_head} );
		print ", {reference}=";
